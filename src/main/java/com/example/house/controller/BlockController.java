...
public class BlockController {
    ...
    @GetMapping
    public ResponseEntity<List<BlockDtoResponse>> getAllBlocks(@RequestParam("page") int page,
                                                               @RequestParam("size") int size) {
        return new ResponseEntity<>(blockService.getAllBlocks(page, size), HttpStatus.OK);
    }

    @GetMapping("{id}")
    public ResponseEntity<BlockDtoResponse> getBlockById(@PathVariable Long id) {
        return new ResponseEntity<>(blockService.getBlockById(id), HttpStatus.OK);
    }

    @GetMapping("{id}/flats")
    public ResponseEntity<List<FlatDtoResponse>> getBlockFlats(@PathVariable Long id) {
